"""Import generated HUD textures with Unreal UI settings."""
...
SOURCE_DIR = PROJECT_ROOT / "ArtPipeline" / "assets" / "ui_hud" / "output" / "textures"
DESTINATION_PATH = "/Game/UI/Textures/HUD"
...
def main() -> None:
    sources = sorted(SOURCE_DIR.glob("T_UI_HUD_*.png"))
    if len(sources) != 7:
        raise RuntimeError(f"Expected 7 HUD textures, found {len(sources)}")
    ...
    for source in sources:
        ...
        unreal.log(f"HUD texture ready: {asset_path}")
